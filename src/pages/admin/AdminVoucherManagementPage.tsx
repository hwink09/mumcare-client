...
export function AdminVoucherManagementPage({ isEmbedded = false }: AdminVoucherManagementPageProps) {
  ...
  const handleDelete = async () => {
    if (!voucherPendingDelete) return;

    setDeleteSubmitting(true);
    try {
      await couponService.delete(voucherPendingDelete._id);
      await loadVouchers();
      toast.success(`Deleted voucher "${voucherPendingDelete.name}" successfully.`);
      setVoucherPendingDelete(null);
    } catch (err) {
      const msg = getErrorMessage(err, "Failed to delete voucher.");
      setError(msg);
      toast.error(msg);
    } finally {
  ...
  return wrapContent(
    <>
      ...
      <ConfirmDialog
        open={Boolean(voucherPendingDelete)}
        title="Delete Voucher"
        description={
          voucherPendingDelete
            ? `Are you sure you want to delete voucher "${voucherPendingDelete.name}"?`
